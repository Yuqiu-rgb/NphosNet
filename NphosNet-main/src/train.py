...
def training(fold, model, device, epochs, criterion, optimizer,
    ...
    for epoch in range(epochs):
        ...
        if auc - max_performance > 1e-4 and epoch > 50:
            ...
            '''PH'''
            # save_model_test(model.state_dict(), fold, auc, '../model/PH_train',
            #                 parameters.learn_name)
            # # Open the file using "append" mode to add content to it
            # with open("../model/PH_train/save_result.txt", "a") as f:
            ...
            #     f.write(result_str)
            '''PK'''
            # save_model_test(model.state_dict(), fold, auc, '../model/PK_train',
            #                         parameters.learn_name)
            # with open("../model/PK_train/save_result.txt", "a") as f:
            #     # Format the content to be written as a string
            #     result_str = "save model: epoch {} - iteration {}: average loss {:.3f} val_acc {:.3f} val_auc {:.3f} learning rate {:.2e}\n".format(
            #                 epoch + 1, step + 1, running_loss, acc, auc,
            #         optimizer.param_groups[0]['lr'])
            ...
            #     f.write(result_str)
            '''PR'''
            save_model_test(model.state_dict(), fold, auc, '../model/PR_train',
                                    parameters.learn_name)
            with open("../model/PR_train/save_result.txt", "a") as f:
                    # Format the content to be written as a string
                    result_str = "save model: epoch {} - iteration {}: average loss {:.3f} val_acc {:.3f} val_auc {:.3f} learning rate {:.2e}\n".format(
                            epoch + 1, step + 1, running_loss, acc, auc,
                            optimizer.param_groups[0]['lr'])
...
def train_validation(parameters,
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(x_train_encoding, train_label.cpu())):
        model = NphosNet.BERT(parameters).to(device, non_blocking=True)
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--PH', action='store_true', help='Select the PH dataset')
    # parser.add_argument('--PK', action='store_true', help='Select the PK dataset')
    # parser.add_argument('--PR', action='store_true', help='Select the PR dataset')
    ...
    # x_test_encoding = BERT_encoding(x_test, x_train).to(device, non_blocking=True)
    #
    # x_train_embedding, x_test_embedding = embedding_load_pk()
    # x_train_str_embedding, x_test_str_embedding = embedding_str_load_pk()
    #
    '''PR '''
